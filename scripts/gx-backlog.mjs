...
function loadComponent(root, component) {
  const stages = {};
  for (const stage of Object.keys(STAGES)) stages[stage] = load(root, component, stage);
  return stages;
}
...
switch (cmd) {
  ...
  case "show": {
    ...
    const stages = loadComponent(root, component);
  ...
  case "add": {
    const component = need("component");
    const { items, meta } = load(root, component, "backlog");
  ...
  case "move": {
    ...
    if (!from) die(`${id} is not in any stage of ${component}`);
  ...
  default:
    die(
      `unknown command ${cmd ?? "(none)"}. One of: detect, create, show, ready, add, move, rebuild`,
    );
}
