...
Hand off to the **deployment-workmate** agent to run the **full deploy flow** for this repository. You are an entry point only — the agent owns the reasoning and the execution. Do not script the deployment yourself.

Tell the agent to run its deploy sequence end to end:
...
7. **Execute** — provision and deploy through the Cloud Run MCP tools (`deploy-local-folder` / `deploy-file-contents`). Never tell me to open the GCP console.
