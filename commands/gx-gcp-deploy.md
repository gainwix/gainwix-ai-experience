---
name: gx-gcp-deploy
description: Deploy this app to Google Cloud Run through the GainWix workmate — resolve GCP access (sign-in + project + region), detect, plan, gate, deploy, document. The headline command; it owns all the GCP setup.
...
3. **Detect & classify** — language/framework/runtime/build/port/env; classify production vs non-production and state why.
4. **Plan** — required GCP infra (Cloud Run first; flag DB/networking/IAM); human-readable plan with estimated cost and the diff from current state.
...
8. **Execute** — provision and deploy through the Cloud Run MCP tools (`deploy_local_folder` / `deploy_file_contents`). Never tell me to open the GCP console.
